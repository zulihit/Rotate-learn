...
class KGEModel(nn.Module):
    ...
    def TransE(self, head, relation, tail, mode):
        if mode == 'head-batch':
            ...
            score = (head + relation) - tail # (1024,1,1000)+(1024,1,1000)-(1024,256,1000)=(1024,256,1000)
        score = self.gamma.item() - torch.norm(score, p=1, dim=2)  # {torch.norm：对输入的Tensor求p范数  (1024,256,1000)>>(1024,256)}  然后用gamma减去每（1024，256）的每一个值返回（1024，256）
        return score
    ...
    def test_step(model, test_triples, all_true_triples, args):
        ...
        if args.countries: # 针对论文中的country数据集
            ...
            for metric in logs[0].keys():
                metrics[metric] = sum([log[metric] for log in logs])/len(logs) # 对log的每一个metric都要求和
